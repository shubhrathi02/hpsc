...
// =================================================================================== //
  int total_iter = 0;
#pragma omp parallel shared(total_iter)
  {
    ...
    while ( thisThreadConverged != 1/* TO-DO */  && ++iter <= max_iter )  
      {
...
//	  if ( numTHconverged == numTH ) printf("Jacobi converged in %d iterations.\n",iter);
	...
	for ( int row = Lower ; row <=  Upper; ++row/*  Global matrix loop bounds for int row */ )    
      ...
      total_iter = std::max(total_iter, iter);
...
// =================================================================================== //
  ...
  /*for (int i = 0; i < numTH; ++i) {
    if(THconverged[i] == 0) {
      printf("WARNING: Jacobi did not converge.\n");
    } else {
      printf("Jacobi converged in %d iterations.\n", total_iter);
    }
  }*/

  if (total_iter > nField * 10) {
    printf("WARNING: Jacobi did not converge.\n");
  } else {
    printf("Jacobi converged in %d iterations.\n", total_iter);
  }

  //if ( numTHconverged != numTH ) 
