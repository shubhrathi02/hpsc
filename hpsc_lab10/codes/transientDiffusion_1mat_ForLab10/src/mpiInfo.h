...
class mpiInfo
{
 public:
  ...
  void ExchangeBoundaryInfo_EW(VD &field)
  {
    int size_y = nRealy+1;
    
	// Store into comm array
	
	jLOOP commR_s[j] = field[ pid( nRealx   , j       ) ];

	if ( nei_e >= 0 ) jLOOP printf("myPE: %d Sending commR_s[%d] = %9.2f to %d\n",myPE,j,commR_s[j],nei_e);
	
	// East-West Send/Receive
	
	if ( nei_e >= 0 )   err = MPI_Isend(commR_s, size_y , MPI_DOUBLE , nei_e , tag , MPI_COMM_WORLD , &request ); 
	if ( nei_w >= 0 ) { err = MPI_Irecv(commL_r, size_y , MPI_DOUBLE , nei_w , tag , MPI_COMM_WORLD , &request );   MPI_Wait(&request,&status); }

	// Get from comm array
	
	if ( nei_w >= 0 ) jLOOP printf("myPE: %d Received commL_r[%d] = %9.2f from %d\n",myPE,j,commL_r[j],nei_w);
	if ( nei_w >= 0 ) jLOOP field[ pid( 1   , j   ) ] = commL_r[j];
	
  }



  void ExchangeBoundaryInfo_NS(VD &field)
  {
    int size_x = nRealx+1;
    
	// Put int comm array
	
	iLOOP commT_s[i] = field[ pid(    i     , nRealy  ) ]; 

	if ( nei_n >= 0 ) iLOOP printf("myPE: %d Sending commT_s[%d] = %9.2f to %d\n",myPE,i,commT_s[i],nei_n);
	
	// Send/Receive

	if ( nei_n >= 0 )   err = MPI_Isend(commT_s, size_x , MPI_DOUBLE , nei_n , tag , MPI_COMM_WORLD , &request );
	if ( nei_s >= 0 ) { err = MPI_Irecv(commB_r, size_x , MPI_DOUBLE , nei_s , tag , MPI_COMM_WORLD , &request );   MPI_Wait(&request,&status); } 
	
	// Get from comm array

	if ( nei_s >= 0 ) iLOOP printf("myPE: %d Received commB_r[%d] = %9.2f from %d\n",myPE,i,commB_r[i],nei_s);
	if ( nei_s >= 0 ) iLOOP field[ pid( i   , 1   ) ] = commB_r[i];
