...
void Jacobi(VD &Solution , mpiInfo &myMPI )
  {
    int converged;
    int iter = 0;
    double newval;
...
void CG(VD &Solution , mpiInfo & myMPI)
{
  ...
  while ( global_converged == 0  && ++iter <= max_iter)
    ...
    if ( global_converged == 1 ) if ( myMPI.myPE == 0 ) cout << "CG converged in " << iter << " iterations.\n" ;
    if ( global_converged == 0 ) if ( myMPI.myPE == 0 ) cout << "CG failed to converge aftger " << iter << " iterations.\n" ;
